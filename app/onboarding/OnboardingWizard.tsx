...
function radioCardClass(selected: boolean) {
  return [
    "relative w-full rounded-2xl border-2 bg-white p-5 text-left",
    "transition-all duration-150 hover:shadow-md hover:scale-[1.02] cursor-pointer",
    selected ? "border-purple-600 shadow-md" : "border-zinc-200",
  ].join(" ");
}

function multiCardClass(selected: boolean) {
  return [
    "relative w-full rounded-2xl border-2 bg-white p-5 text-left",
    "transition-all duration-150 hover:shadow-md hover:scale-[1.02] cursor-pointer",
...
export default function OnboardingWizard({ skills }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const data = await res.json() as { success?: boolean; pathId?: string; reason?: string; error?: string };

      // Free-tier path limit hit — show upgrade modal
      ...
      <div className="w-full max-w-3xl">
        ...
        {step === 4 && (
          <div>
            <h1 className="font-heading text-4xl font-bold text-zinc-900 text-center mb-2">
              What&apos;s your budget for learning this skill?
            </h1>
              ...
              {BUDGET_OPTIONS.map(({ value, emoji, label, description }) => {
                ...
                return (
                  <button
                    ...
                    className={radioCardClass(isSelected)}
                  >
                    {isSelected && (
        ...
        {step === 5 && (
          <div>
            ...
            <p className="text-zinc-500 text-center mb-8">
              Pick all that apply — most people are a mix
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {STYLE_OPTIONS.map(({ value, emoji, label, description }) => {
                ...
                return (
                  <button
                    ...
                    className={multiCardClass(isSelected)}
                  >
                    {isSelected && (
                      <span className="absolute top-3 right-3 bg-purple-600 text-white rounded-full p-0.5">
                        <Check className="w-3 h-3" />
        ...
        {step === 6 && (
          <div>
            ...
            <p className="text-zinc-500 text-center mb-8">
              We&apos;ll pace your path to fit your deadline.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
